...
### ゲーム数カウントと連チャン(非自明)
ゲーム数の専用信号は無い. `game_counter.GameCounter`(`session_manager` と同様の単一インスタンス)が IN パルスを時間ギャップ(`IN_BURST_GAP_SEC`)で区切り、1 かたまり = 1 ゲームとして直近ボーナスからのゲーム数を数える. 1 ゲーム分の賭けメダルは連続で入り、ゲーム間にはリール回転ぶんの間隔が空くため、賭け枚数が変わっても(通常 3 枚 / ジャグラーのペカリ後 1 枚)枚数に依存せず数えられる. BB/RB で 0 にリセット. サーバ再起動時は `seed_from_db()` が events テーブルから復元.

連チャン = 直近ボーナスから `RENCHAN_LIMIT`(100)ゲーム以内の次のボーナス当選. BB/RB イベントに `renchan` フラグと当選時ゲーム数 `win_game_count` を載せ、フロントは 100G 以内のあいだ連チャンゾーン表示を出し、ゾーン内当選で連チャン専用演出(金色)を出す.

注意: リプレイ成立中はメダル投入が無く IN が出ないため、リプレイ分のゲームは数えられない(IN/OUT/RB/BB の 4 信号だけでは原理的に検出不可).

### SSE イベント形式
クライアントは EventSource で `/api/events/stream` を購読. 内部メッセージは `kind` フィールドで判別:
- `event` … パチスロイベント(IN/OUT/BB/RB). `game_count` / `in_renchan_zone` を伴う. BB/RB はさらに `renchan` / `win_game_count` を持つ
- `session_start` / `session_end`
- `register_required` / `user_registered`
- `settings_updated` … ユーザーのパネル表示プロファイル更新

初回接続時のみ別 SSE event 名 `snapshot` で累計カウント + `game_count` / `in_renchan_zone` を送る.
