...
### 過去セッションのグラフ再描画(非自明)
`events.game_id`(Pico の暫定ゲーム番号)を全イベントに記録しているので、過去セッションの events からスランプ・払い出しグラフを再構成できる. `db.build_session_series` が events を game_id でゲームに区切り、スランプ系列(各ゲーム終了時の累計差枚)と払い出し系列(OUT を `PAYOUT_GAP_MS` で区切り直したもの)を作る. `GET /api/sessions/{id}/series` で取得し、フロントはセッション履歴パネルの行クリックで再描画する. 生イベントはそのまま残す方針(累計カウントが events 集計のため. DB は約数MB/セッション増えるが Pi5 なら数年余裕、剪定が要れば別途).
...
### DB スキーマ
`host/data/events.db` (SQLite). `host/app/db.py` の `SCHEMA` 定数に DDL がまとまっている. レガシー DB(`session_id` / `game_id` 列がない `events`、`display_settings` 列がない `users`)は `init_db()` 時に自動 ALTER で吸収.

```
events    (id, ts, type, session_id, game_id)
