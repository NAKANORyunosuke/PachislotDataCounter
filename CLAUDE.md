...
### ゲーム数カウントと連チャン(非自明)
ゲーム数の専用信号は無い. `game_counter.GameCounter`(`session_manager` と同様の単一インスタンス)が IN パルスを時間ギャップ(`IN_BURST_GAP_SEC`)で区切り、1 かたまり = 1 ゲームとして直近ボーナスからのゲーム数を数える. IN はレバーON 時にまとまって出る(リプレイの自動再ベットも IN を出す)ため、賭け枚数が変わっても(通常 3 枚 / ジャグラーのペカリ後 1 枚)、またリプレイゲームも、種別に依存せず数えられる. サーバ再起動時は `seed_from_db()` が events テーブルから復元.

ボーナス中もベットして IN が出るため、放っておくとボーナス中のゲームも数えてしまう. `host/config.json` の `bonus_games`(BB/RB 別のボーナス1回あたりゲーム数)を設定すると、ボーナス当選時にカウントを `-bonus_games` から開始し、ボーナス中は表示ゲーム数 0・ボーナス終了あたりで 1 から数え直す(`0` のままなら従来どおりボーナス中も加算). 外部に見せる値は負値を 0 でクランプ.
