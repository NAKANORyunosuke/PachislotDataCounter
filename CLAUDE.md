...
Pico は USB シリアルに CSV 生ログを 1 行 1 イベントで送る. 起動時に `READY,format=v1,fields=timestamp_ms,game_id,event,edge,seq` ヘッダを出し、以降は `timestamp_ms,game_id,event,edge,seq`. `event` は IN/OUT/RB/BB、`edge` は FALL(HIGH→LOW)/ RISE(LOW→HIGH)で両エッジを送る. `game_id` は Pico が IN FALL の 300ms ギャップで振る暫定ゲーム番号. `serial_reader.py` の `parse_csv_line` がパースし、DB 記録・SSE 配信はイベント本体の **FALL のみ**(RISE は `game_counter` がボーナス窓追跡に使うだけ). Pico は生ログを出すだけで、ゲーム結果やボーナス状態の確定はしない.
