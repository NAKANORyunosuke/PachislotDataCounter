#!/usr/bin/env bash
# Copy pico-edge-reader/main.py onto the connected Raspberry Pi Pico via
# mpremote, then soft-reset so the new code takes effect immediately.
#
# Usage:
#   bash pico-edge-reader/scripts/flash.sh             # auto-detect serial device
...
if [[ ! -x "$MPREMOTE" ]]; then
  echo "mpremote not found. Run pico-edge-reader/scripts/setup.sh first." >&2
  exit 1
fi
